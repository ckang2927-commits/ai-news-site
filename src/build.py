...
def content_card(title, desc, link, tags, date_str):
    ...
    date_html = f'<span style="color:var(--text-muted);font-size:0.72rem;">{html_escape(date_str)}</span>' if date_str else ""
    return f'''<a href=\"{link}\" class="content-card">
  <div class="flex items-start justify-between gap-3">
    <div class="flex-1 min-w-0">
      <h3 class="line-clamp-2">{html_escape(title)}</h3>
      <p style="color:var(--text-muted);font-size:0.78rem;line-height:1.5;margin-top:0.4rem;" class="line-clamp-2">{html_escape(desc[:120])}</p>
...
def list_page(title, desc, cards_html, nav_d=1):
    np = nav_prefix(nav_d)
    body = f'''<div class="mb-8 animate-in">
  <a href=' + np + '/index.html\" class="back-btn">&larr; 返回首页</a>
  <h1 style="font-family:'Orbitron',sans-serif;font-size:1.5rem;font-weight:700;letter-spacing:1px;margin-top:0.5rem;background:var(--gradient-2);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;">{html_escape(title)}</h1>
...
def detail_page(title, content_html, source="", nav_d=1):
    np = nav_prefix(nav_d)
    body = f'''<div class="flex items-center gap-3 mb-6 animate-in">
